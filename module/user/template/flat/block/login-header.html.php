{if Phpfox::getLib('module')->getFullControllerName() != 'user.login'}
									...
									{/if}
										<form method="post" action="{url link='user.login'}">
											...
											<div class="header_menu_login_button">
												...
												<div class="close"><a href="javascript:void(0)" onclick="{literal}$('#header_menu_login').animate({top:'-75px'},500);{/literal}">close</a></div>
											</div>
										</form>
									{if isset($bCustomLogin)}
									</div>									
                                ...
                                {if Phpfox::getLib('module')->getFullControllerName() != 'user.register-sso'}
								<div id="login_links">
									<ul>
										<li class="signup_link"><a href="{url link='user.register'}">{phrase var='user.sign_up'}</a></li>
										<li class="login_link">
                                            <a href="javascript:void(0)" onclick="{literal}$('#header_menu_login').animate({top:'8px'},500);{/literal}">{phrase var='user.login_singular'}</a>
                                        </li>
                                        {if $showSsoButton}
                                        <li class="login_link">
                                            <a href="{$sLink}" >{phrase var='user.sso_login_singular'}</a>
                                        </li>
                                        {/if}
									</ul>
								</div>
