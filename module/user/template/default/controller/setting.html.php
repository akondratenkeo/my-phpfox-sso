	...
	<form method="post" action="{url link='user.setting'}" id="js_form" onsubmit="{$sGetJsForm}">		
		...
		{if !Phpfox::getUserBy('fb_user_id')}
			<div class="table">
				...
				<div class="table_right">
					<div id="js_password_info" style="padding-top:2px;">
                        {if !$isSsoUser}
                            <a href="#" onclick="tb_show('{phrase var='user.change_password' phpfox_squote=true}', $.ajaxBox('user.changePassword', 'height=250&amp;width=500')); return false;">{phrase var='user.change_password'}</a>
                        {else}
                            {phrase var='user.type_of_your_account_dont_support_password_changing'}
                        {/if}
                    </div>
				</div>
				<div class="clear"></div>
