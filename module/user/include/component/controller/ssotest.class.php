 ...
 * @author			dev
 * @package  		Module_User
 * @version 		$Id: ssotest.class.php 5463 2015-11-18 $
 */
class User_Component_Controller_Ssotest extends Phpfox_Component {
    /**
        ...
        if (!$isAuthenticated){
            if(Phpfox::getLib('request')->get('req4') == 'login'){
                $auth->requireAuth(array(
                    'ReturnTo' => 'https://baylor.g9md.net/user/ssotest',
                    'KeepPost' => FALSE,
                ));
            }
        } else {
    ...
    {
        (($sPlugin = Phpfox_Plugin::get('user.component_controller_ssotest_clean')) ? eval($sPlugin) : false);
    }
}
