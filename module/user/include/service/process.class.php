...
{	
		...
		*/
		...
		if (!defined('PHPFOX_INSTALLER'))
		{
			...
			if ( (!defined('PHPFOX_INSTALLER') && Phpfox::getParam('user.verify_email_at_signup') == false && !isset($bDoNotSendWelcomeEmail)) || isset($bSendWelcomeEmailPlg))
			{
                ...
                if (isset($aVals['isSso']) && $aVals['isSso'] === true ) {
                    $bWelcomeMsg = 'core.welcome_sso_email_content';
                } else {
                    $bWelcomeMsg = 'core.welcome_email_content';
                }

                Phpfox::getLib('mail')
					->to($iId)
					->subject(array('core.welcome_email_subject', array('site' => Phpfox::getParam('core.site_title'))))
					->message(array($bWelcomeMsg))
					->send();
			}
        ...
        if (isset($aVals['isSso']) && $aVals['isSso'] === true && isset($bSkipVerifyEmail) && $bSkipVerifyEmail === true) {
            (Phpfox::isModule('feed') ? Phpfox::getService('feed.process')->allowGuest()->add('userjoined_joined', $iId, null, null, null, $iId) : null);
        }
        ...
        $this->database()->insert(Phpfox::getT('user_notification'), array(
                'user_id' => $iId,
                'user_notification' => 'newsletter.can_receive_notification'
            )
        );
		return $iId;
	}
	...
	public function uploadImage($iId, $bForce = true, $sPath = null, $bNoCheck = false, $iPhotoId = 0)
	{
		...
		else
		{
			...
			if (Phpfox::getParam('core.allow_cdn'))
			{
                ...
                if (isset($oImage->sPath) && !empty($oImage->sPath)) {
                    if(file_exists($oImage->sPath)){
                        @copy($oImage->sPath, Phpfox::getParam('core.dir_user'). sprintf($sFileName, '_480'));
                        @copy($oImage->sPath,str_replace('.jpg', '_480.jpg', $oImage->sPath));
                    }
                    Phpfox::getLib('cdn')->put(Phpfox::getParam('core.dir_user'). sprintf($sFileName, '_480'));
                    Phpfox::getLib('cdn')->put(str_replace('.jpg', '_480.jpg', $oImage->sPath));
                }
			}
	...
	public function updateStatus($aVals)
	{
		...
		if (isset($aVals['privacy']) && $aVals['privacy'] == '4')
		{
            ...
            if (isset($aVals['privacy_list']) && !empty($aVals['privacy_list'])) {
                $sLink = Phpfox::getLib('url')->makeUrl(Phpfox::getUserBy('user_name'), array('status-id' => $iStatusId));

                if(is_array($aVals['privacy_list'])){
                    $aVals['privacy_list'] = implode(',', $aVals['privacy_list']);
                }

                $users_in_list = Phpfox::getLib('database')
                    ->select("friend_user_id")
                    ->from(Phpfox::getT('friend_list_data'))
                    ->where("list_id IN (" . $aVals['privacy_list'] . ")")
                    ->execute('getRows');

                foreach ($users_in_list as $user_in_list) {

                    if (Phpfox::isModule('notification')) {
                        Phpfox::getService('notification.process')->add('user_customadd', $iStatusId, $user_in_list['friend_user_id']);
                    }

                    Phpfox::getLib('mail')->to($user_in_list['friend_user_id'])
                        ->subject(
                            array(
                                'user.full_name_shared_status_for_custom_list',
                                array('full_name' => Phpfox::getUserBy('full_name'))
                            )
                        )
                        ->message(
                            array(
                                'user.full_name_shared_status_for_custom_list_to_view_follow_the_link',
